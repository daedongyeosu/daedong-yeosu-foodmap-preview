...
assert.match(eventJs, /function scheduleCampaignFollowup\(\)[\s\S]*?daedongMukkebiReadyAt = readyAt[\s\S]*?openEvent\(\{automatic: true\}\)/,
...
assert.match(html, /mukkebi-summer-event\.js\?v=[^"\n]*no-late-interrupt-3-scroll-cancel-1-layer-guard-1[^"]*kakao-opening-touch-1-startup-order-1-natural-followup-1-mukkebi-second-1/);
