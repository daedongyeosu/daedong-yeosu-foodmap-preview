...
<html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(page.title)} | 여수맛지도</title><meta name="description" content="${esc(page.sections[0].paragraphs[0])}"><link rel="stylesheet" href="../page.css?v=1"><script src="../page.js?v=1" defer></script></head>
<body><header><a href="/" id="back">← 음식지도로</a><span>여수맛지도</span></header><main><h1>${esc(page.title)}</h1>${sections}<section><h2>사진과 영상</h2><div class="gallery">${gallery}</div></section></main><nav class="actions" aria-label="문의 및 위치"><a href="tel:${esc(page.phone)}">전화 문의 ${esc(page.phone)}</a>${page.mapUrl?`<a href="${esc(page.mapUrl)}" rel="noopener">네이버지도</a>`:''}</nav></body></html>\n`;
