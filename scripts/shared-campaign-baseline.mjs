...
const allStoreBatch=new Set(JSON.parse(fs.readFileSync('data/all-store-campaign-coverage.json','utf8')).addedCampaignIds);
...
export function beforeSharedHero(hero){
 ...
 return {...hero,campaigns:Object.fromEntries(Object.entries(hero.campaigns)
  .filter(([id])=>!added.has(id)&&!allStoreBatch.has(id)).map(([id,c])=>[id,!production&&id==='068b2ae8fe32874a'?fixture.previewOriginalPizza:fixture.originalCampaigns[id]
   ? {...fixture.originalCampaigns[id],...(!production?{layout:'food14-plus3'}:{})}:c]))};
}
export function beforeSharedLinks(links){
 return {...links,campaigns:links.campaigns.filter(x=>!added.has(x.storeId)&&!allStoreBatch.has(x.storeId))};
}
