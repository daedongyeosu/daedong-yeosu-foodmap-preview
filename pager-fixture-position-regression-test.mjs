...
assert.match(source, /Math\.abs\(revealedPage\.scrollY - beforeSwipe\.scrollY\) < 2/);
assert.match(source, /transitionMs < 250/);
assert.match(source, /Math\.abs\(afterRanking\.gridTop - beforeRanking\.gridTop\) < 2/);
console.log('pager fixture positioning: PASS (instant setup; original latency/position checks unchanged)');
