...
function overviewSearchText(entry) {
  ...
  function overviewCardMarkup(entry) {
    ...
    const storeImage = typeof isOfficialStorePlaceholderImage === 'function' && isOfficialStorePlaceholderImage(resolvedStoreImage)
      ? matchedMenuImage
    ...
    const branchBadge = typeof storeBranchBadgeMarkup === 'function'
      ? storeBranchBadgeMarkup(entry.store, 'store-service-branch-badge')
      : '';
      ...
      <article class="store-service-overview-group">
        <button type="button" class="store-service-overview-card" data-store-service-store-id="${escapeHtml(entry.storeId)}">
          ...
          <span class="store-service-overview-card-main">
            <strong title="${escapeHtml(entry.store?.name || '가게 정보')}">${escapeHtml(entry.store?.name || '가게 정보')}</strong>
            <small>${branchBadge}${escapeHtml(entry.area)} · ${escapeHtml(formatCustomerHours24(hoursSummary))}</small>
          </span>
