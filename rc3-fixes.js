...
function rc3EnhanceStoreDetail(store) {
  ...
  const direct = channels.primaryOrder.directOrder
    ? routeLink(channels.primaryOrder.directOrder, 'local-order-route')
    : '';
  const community = [channels.primaryOrder.mukkebi, channels.primaryOrder.ddangyo, channels.primaryOrder.ondongne]
    .filter(Boolean)
    .map(route => routeLink(route, 'local-order-route'))
    .join('');
  ...
  gallery?.insertAdjacentHTML('afterend', `${utilities ? `<div class="detail-quick-links">${utilities}</div>` : ''}<div class="detail-routes local-detail-routes">${direct}${apps}${community}${phone || (!direct && !apps && !community ? '<p class="muted">등록된 주문방법을 확인 중입니다.</p>' : '')}</div>${other}`);
}
