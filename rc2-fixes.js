...
fxAppBrowserMarkup = function rc2AppBrowserMarkup(key, selectedCategory = '추천') {
  ...
  const cats = [...new Set(all.map(store => store.cat).filter(Boolean))];
  ...
  return `<section class="app-browser"><header class="app-browser-head">${appIcon(key, 'app-browser-head-icon')}<div><h2 id="modalTitle">${escapeHtml(meta.label)} 등록 가게</h2><p>실제 주문주소가 등록된 가게만 보여드립니다.</p></div></header>${chips}${rc2SelectedCategoryMarkup(selectedCategory)}<div class="app-browser-list">${cards || '<div class="empty">해당 조건의 가게가 없습니다.</div>'}</div></section>`;
};
...
function rc2OpenOtherApps() {
  openModal(`<section class="app-browser other-apps-modal"><h2 id="modalTitle">다른 주문앱</h2><p>이용할 주문앱을 선택해 등록된 가게를 확인하세요.</p><div class="other-app-choice-grid"><button type="button" class="glass-action" data-global-external="yogiyo"><img src="assets/yogiyo.jpg" alt=""><b>요기요</b></button><button type="button" class="glass-action" data-global-external="coupang"><img src="assets/coupang-eats.jpg" alt=""><b>쿠팡이츠</b></button><button type="button" class="glass-action" data-global-external="baemin"><img src="assets/baemin.jpg" alt=""><b>배달의민족</b></button></div></section>`);
}
...
fxOpenBrandHub = function rc2OpenBrandHub(view = 'channels', value = '') {
  ...
  if (view === 'direct') {
    ...
    openModal(`<section class="brand-app-hub direct-brand-browser"><h2 id="modalTitle">직접 브랜드앱</h2><p>현재 검증된 링크는 Android Google Play입니다. iPhone은 자동 이동하지 않습니다.</p>${rc2BrandCategoryChips(category)}<div class="brand-app-grid">${cards}</div></section>`);
    return;
  }
