...
assert.match(rc3, /const rc3OrderMethodsPointers = new Map\(\)/, '주문방법 터치를 문서 단위로 추적해야 합니다.');
...
assert.match(rc3, /window\.addEventListener\('pageshow', rc3ResetOrderMethodsTouchState, true\)/, '외부 앱 복귀 시 남은 포인터 상태를 초기화해야 합니다.');
...
assert.ok(
  rc3.indexOf('Date.now() < rc3OrderMethodsGhostClickUntil', clickGuardIndex) > clickGuardIndex,
  '터치 뒤 합성 클릭만 해당 주문방법 버튼에서 차단해야 합니다.'
);

for (const [name, source] of [['final-experience.js', finalExperience], ['index.html', index]]) {
  assert.match(source, /order-methods-return-touch-3/, `${name} 캐시 버전이 갱신되어야 합니다.`);
  assert.doesNotMatch(source, /order-methods-mobile-touch-2/, `${name}에 이전 캐시 버전이 남으면 안 됩니다.`);
}
