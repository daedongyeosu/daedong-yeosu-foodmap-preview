...
const FX_HOME_SHARE_URL=FX_REGION.code==='yeosu'?'https://daedongmap.com'+(location.pathname==='/s'?'/s':'/'):new URL(window.daedongRegionUrl?.(FX_REGION.code)||window.location.href,window.location.origin).href;
...
function fxRankStores(spec){return stores.filter(fxVisible).filter(fxHasCustomerAction).filter(store=>fxThemeMatch(store,spec)).map(store=>{const distance=fxDistance(store);const low=['direct','mukkebi','ddangyo','ondongne'].some(key=>storeHasChannel(store,key));let score=spec.pattern?80:20;if(distance!==null)score+=Math.max(0,32-distance*4);if(low)score+=12;if(store.managed)score+=8;else if(store.sharedManaged)score+=5;if(spec.kind==='near'&&distance!==null)score+=Math.max(0,120-distance*25);if(spec.kind==='local'&&low)score+=80;if(spec.kind==='new')score+=Number(store.rawIndex)||0;if(store.deprioritized)score-=100000;return{store,distance,score};}).sort((a,b)=>compareStoreBusinessStatus(a,b)||b.score-a.score||(a.distance??999)-(b.distance??999)||a.store.name.localeCompare(b.store.name,'ko')).map(item=>({...item.store,distance:item.distance}));}
const FX_RAIL_SPECS=[
...
function fxOpenStoreShare(store,target){
 ...
 openModal(`<section class="home-share-sheet store-share-sheet" data-store-share-id="${escapeHtml(store.id)}">
  ...
  <div class="home-share-preview store-share-preview">${photo?`<img src="${escapeHtml(photo)}" alt="${escapeHtml(store.name)}">`:`<img src="assets/app-icons/daedong-share-lightning-20260909.png" alt="${escapeHtml(FX_MAP_NAME)}">`}<span><b>${escapeHtml(store.name)}</b><small>가게를 바로 여는 ${escapeHtml(FX_MAP_NAME)} 주소</small></span></div>
...
function fxOpenHomeShare(target){
 ...
 openModal(`<section class="home-share-sheet">
  ...
  <div class="home-share-preview"><img src="assets/app-icons/daedong-share-lightning-20260909.png" alt=""><span><b>${escapeHtml(FX_MAP_NAME)}</b><small>${FX_HOME_SHARE_URL}</small></span></div>
 ...
 fxRc3Script.onload=()=>{
  ...
  fxRc4Script.onload=()=>{
   ...
   fxRc5Script.onload=()=>{
    ...
    const script=document.createElement('script');script.src='rc6-fixes.js?v=hero-store-direct-1-multi-category-1-hamburger-priority-1-pizza-priority-2-kongsanso-store-family-1-store-badge-removed-1-handsu-copy-spacing-1-hero-card-cta-removed-1-rain-mode-admin-1-local-channel-marker-1-location-coordinate-merge-1-business-status-ranking-1-release-readiness-1-hero-open-only-1-hero-area-label-removed-1-three-main-ads-restored-1-notion-hero-return-1-goheung-isolation-2-instant-hero-loading-1-keep-placeholder-1-coordinate-yield-1-pager-stable-refresh-1-hero-photo-recovery-1-store-campaign-nine-2-tamnaneun-menu-hero-4-store-campaign-standard-1-auto-menu-fill-all-1-domino-munsu-14-1-hero-stable-height-1-manual-carousels-1-former-managed-bottom-1-four-store-qr-1-four-store-14-plus-3-1-hide-tamnaneun-1-teum-campaign-1-three-store-campaign-1-haeinine-campaign-1-native-pages-1-baeknyeon-ended-1-collected-shopinshop-1-managed-business-1-shared-referral-2-shared-short-1-ajuker-munsu-14-1-shared102-menu-1-active-viewport-anchor-1-merged-campaign-alias-1-store-entry-status-20260913-photo-address-priority-20260913';
    script.onload=()=>{
     const addressScript=document.createElement('script');addressScript.src='rc7-address-map.js?v=address-home-return-1-coarse-region-1-inapp-location-recovery-1-outside-yeosu-full-list-1-saved-address-first-1-release-readiness-1-step-touch-back-1-photo-address-priority-20260913';
