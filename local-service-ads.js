...
(() => {
  ...
  function detailContent(id) {
    ...
    if (ad.theme === 'insurance') return `<section class="local-service-detail local-service-insurance insurance-direct" data-service-detail="${ad.id}"><span class="local-service-kicker">보험 상담 <span class="local-service-disclosure">광고</span></span>${insuranceBrochure(ad)}<div class="local-service-contact"><span>${escape(ad.person)}</span><strong>${ad.phone}</strong></div><nav class="local-service-actions" aria-label="${escape(ad.person)} 상담"><a class="local-service-primary" href="tel:${ad.phone.replace(/-/g, '')}">담당자에게 전화하기</a><a href="sms:${ad.phone.replace(/-/g, '')}">문자로 문의하기</a><a class="local-service-official" href="${escape(ad.official)}" target="_blank" rel="noopener noreferrer">현대해상 모바일 명함 보기 ↗</a></nav><footer class="insurance-source-links" aria-label="안내 출처"><span>자료 출처</span><a href="${insuranceDocument.source}" target="_blank" rel="noopener noreferrer">현대해상 Hi2601 상품요약서 ↗</a><span>담당자 제공 전단 ${ad.pages.map((src, i) => `<a href="${src}" target="_blank" rel="noopener" aria-label="제공 전단 ${i + 1}장 보기">${i + 1}장</a>`).join(' · ')}</span></footer></section>`;
