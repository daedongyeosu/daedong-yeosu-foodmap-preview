...
assert.match(index, /store-service-info\.css\?v=store-service-9-customer-search-ux-1/);
assert.match(index, /store-service-info\.js\?v=store-service-13-sensitive-counts-hidden-1/);
...
assert.match(serviceScript, /미확인은 아직 사용 가능 여부가 확인되지 않은 정보/);
...
assert.match(serviceScript, /주문 전 해당 주문앱에서 다시 확인해 주세요/);
...
assert.match(serviceScript, /혜택 옆에 적힌 주문앱에서 이용할 수 있습니다/);
