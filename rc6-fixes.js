...
async function rc6ApplyCoordinates(){for(let index=0;index<canonicalStores.length;index+=1){const store=canonicalStores[index],row=rc6Coordinates[String(store.id)],catalogLat=Number(store.lat),catalogLng=Number(store.lng),hasCatalogCoordinates=store.lat!==null&&store.lat!==''&&store.lng!==null&&store.lng!==''&&Number.isFinite(catalogLat)&&Number.isFinite(catalogLng);if(row&&row.status==='verified'&&Number.isFinite(Number(row.latitude))&&Number.isFinite(Number(row.longitude))){store.lat=Number(row.latitude);store.lng=Number(row.longitude);store.coordinateSource='verified-static';store.coordinateVerified=true;}else if(hasCatalogCoordinates){store.lat=catalogLat;store.lng=catalogLng;store.coordinateSource=store.coordinateSource||'verified-catalog';store.coordinateVerified=true;}else{store.lat=null;store.lng=null;store.coordinateSource='';store.coordinateVerified=false;}const inferred=storeNeighborhoods(store),coordinateNeighborhood=rc6Verified(store)?closestNeighborhoodForCoordinates({lat:store.lat,lng:store.lng}):'';store.neighborhoods=[...new Set([...inferred,...(coordinateNeighborhood?[coordinateNeighborhood]:[])])];store.primaryNeighborhood=store.neighborhoods[0]||'';if((index+1)%48===0&&index+1<canonicalStores.length)await yieldToMainThread();}coordinateStores=canonicalStores.filter(rc6Verified);rc6LocationCache={key:'',stores:[]};}
function rc6ApplyStorePriority(){rc6ManagedStoreIds=new Set((rc6StorePriority.managedStoreIds||[]).map(String));rc6SharedManagedStoreIds=new Set((rc6StorePriority.sharedManagedStoreIds||[]).map(String));rc6DeprioritizedStoreIds=new Set((rc6StorePriority.deprioritizedStoreIds||[]).map(String));categoryPriorityOverrides={...LOCATION_CATEGORY_PRIORITY_OVERRIDES,...(rc6StorePriority.categoryPriorityOverrides||{})};canonicalStores.forEach(store=>{const id=String(store.id);store.managed=rc6ManagedStoreIds.has(id);store.sharedManaged=!store.managed&&rc6SharedManagedStoreIds.has(id);store.deprioritized=rc6DeprioritizedStoreIds.has(id);});rc6LocationCache={key:'',stores:[]};}
function rc6OwnershipTier(store){const id=String(store?.id??store?.store_id??'');return rc6ManagedStoreIds.has(id)?0:rc6SharedManagedStoreIds.has(id)?1:rc6DeprioritizedStoreIds.has(id)?3:2;}
...
function rc6ApplyRainExposure(candidates,limit){
 const rows=[...candidates],ratio=rc6RainManagedRatio();
 if(rc6RainMode==='normal')return rows;
 ...
 for(const group of statusGroups){
  ...
  while(result.length<limit&&(managedIndex<managed.length||generalIndex<general.length)){
   const managedTarget=Math.round((groupPosition+1)*ratio),preferManaged=managedIndex<managedTarget;
   if(preferManaged&&managedIndex<managed.length)result.push(managed[managedIndex++]);
   else if(generalIndex<general.length)result.push(general[generalIndex++]);
   else if(managedIndex<managed.length)result.push(managed[managedIndex++]);
   groupPosition+=1;
  }
...
function rc6RememberNotionHeroReturn(slide){
 const image=slide?.querySelector('img'),notionUrl=slide?.dataset.rc6BannerNotion;if(!image||!notionUrl)return;
 try{const url=new URL(notionUrl,location.href),rawImage=image.getAttribute('src')||image.currentSrc||'',inlineImage=/^data:image\/(?:png|jpe?g|webp|gif|avif);base64,/i.test(rawImage)&&rawImage.length<=500000,imageUrl=inlineImage?rawImage:new URL(rawImage,location.href);if(url.protocol!=='https:'||(!inlineImage&&imageUrl.origin!==location.origin))return;sessionStorage.setItem(RC6_NOTION_HERO_RETURN,JSON.stringify({notionUrl:url.href,image:inlineImage?rawImage:imageUrl.href,alt:image.getAttribute('alt')||'',displayIndex:Number(slide.dataset.heroIndex)||0,savedAt:Date.now()}));}catch{}
}
...
function rc6HeroGroupKey(entry){
 ...
 return [storeBusinessStatusPriority(store),bucket,tier,band].join(':');
}
...
function rc6HeroRequiredChannelKeys(){
 return RC6_HERO_LOW_FEE_CHANNEL_KEYS.filter(key=>{
  const serviceButton=document.querySelector(`[data-order-key="${key}"]`);
  return !serviceButton?.disabled;
 });
}
...
function rc6ManagedStoreHeroEntries(){
 ...
 const candidates=stores.filter(store=>fxVisible(store)&&rc6HeroHasRequiredChannels(store)&&(rc6RainMode==='normal'?rc6OwnershipTier(store)<2:true));
 return rc6ApplyRainExposure(rc6RankCandidatesByCustomerLocation(candidates),RC6_DAILY_STORE_HERO_LIMIT).map((rankedStore,index)=>{
...
function rc6ResolveHeroCampaignStoreId(id){
 ...
 return campaign?String(campaign.storeId||id||''):'';
}
function rc6RequestedHeroCampaign(){
 const params=new URLSearchParams(location.search);
 const requestedId=params.get('hero')||params.get('store');
 const campaign=rc6HeroCampaignForEntryStoreId(requestedId);
 if(!campaign)return null;
 const storeId=String(campaign.storeId||requestedId||'');
...
function rc6CampaignHeroEntries(){
 ...
 const photos=slideDefinitions.map((slide,index)=>{
  const slideStore=rc6CampaignStoreById(slide.storeId||campaign.storeId),src=slide.image;
  if(!slideStore||!src)return null;
  return{
...
function rc6RankCandidatesByCustomerLocation(candidates){const nearby=rc6NearStores();if(!nearby.length)return candidates.map((store,index)=>({store,index})).sort((a,b)=>compareStoreBusinessStatus(a,b)||rc6OwnershipTier(a.store)-rc6OwnershipTier(b.store)||a.index-b.index).map(item=>item.store);const rank=new Map(nearby.map((store,index)=>[String(store.id),index])),details=new Map(nearby.map(store=>[String(store.id),store]));return candidates.map((store,index)=>({store:{...store,...details.get(String(store.id))},index})).sort((a,b)=>compareStoreBusinessStatus(a,b)||Number(rc6OwnershipTier(a.store)===3)-Number(rc6OwnershipTier(b.store)===3)||(rank.get(String(a.store.id))??Infinity)-(rank.get(String(b.store.id))??Infinity)||a.index-b.index).map(item=>item.store);}
function rc6CategoryCandidates(){
 ...
 return stores.map(store=>({store,score:relevance(store,state.query)}))
  ...
  .sort((a,b)=>{
   const statusOrder=compareStoreBusinessStatus(a,b);if(statusOrder)return statusOrder;
...
function rc6InstallChannelLocationSorting(){
 ...
 fxPhoneStores=function rc6LocationPhoneStores(category='추천'){
  ...
  return applyCategoryPriorityOverrides(rc6RankCandidatesByCustomerLocation(items.map(item=>item.store)).map(store=>({...byId.get(String(store.id)),store})),category);
 };
