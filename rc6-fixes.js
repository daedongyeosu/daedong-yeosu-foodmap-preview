...
async function rc6Initialize(){[rc6Coordinates,rc6BannerTargets,rc6StorePriority,rc6HeroCampaigns]=await Promise.all([fetchJson('data/store-coordinates.json',{}),fetchJson('data/banner-targets.json?v=notion-banner-15',{}),fetchJson('data/store-priority.json',{}),fetchJson('data/hero-campaigns.json?v=kongsanso-store-family-1',{campaigns:{},virtualStores:{}})]);rc6PrepareCampaignStores();rc6ApplyCoordinates();rc6ApplyStorePriority();rc4StoreHasRealCoordinates=rc6Verified;fxDistance=store=>state.coords&&rc6Verified(store)?haversine(state.coords,{lat:store.lat,lng:store.lng}):null;const originalRank=fxRankStores,originalFiltered=filteredStores,originalRenderStores=renderStores;fxRankStores=spec=>rc6LocationRankedRail(spec,originalRank);filteredStores=()=>applyCategoryPriorityOverrides(rc6RankCandidatesByCustomerLocation(originalFiltered()),state.category);rc5CategoryStores=rc6CategoryStoresByCustomerLocation;rc4CategoryList=rc6CategoryStoresByCustomerLocation;rc6InstallChannelLocationSorting();useCurrentLocation=rc6UseCurrentLocation;commitAddressSelection=rc6CommitAddress;renderStores=function rc6RenderStores(options={}){rc6RenderHero();return originalRenderStores(options);};rc6RenderHero();rc6WatchHeroDay();rc6HeroEvents();rc6Gulls();renderStores({resetCount:true});fxRenderRails();document.querySelector('.build-mark').textContent='대동여수음식지도 RC6 온라인 검수 후보';}
