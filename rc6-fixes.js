...
function rc6ConfigurePartnerPriority(){
 ...
 rc6PartnerStoreIds=new Set(!RC6_IS_GOHEUNG&&profile?.enabled===true?(profile.storeIds||[]).map(String).filter(id=>!rc6DeprioritizedStoreIds.has(id)):[]);
}
...
const rc6HeroDataPromise=Promise.all([fetchJson('data/store-coordinates.json',{}),fetchJson('data/banner-targets.json?v=native-pages-1',{}),fetchJson('data/store-priority.json?v=former-managed-bottom-1-baeknyeon-ended-1-shared-referral-1',{}),fetchJson('data/hero-campaigns.json?v=store-campaign-standard-1-four-store-qr-1-four-store-14-plus-3-1-teum-campaign-1-three-store-campaign-1-haeinine-campaign-1-collected-shopinshop-1-managed-business-1',{campaigns:{},virtualStores:{}}),rc6LoadRainMode()]);
