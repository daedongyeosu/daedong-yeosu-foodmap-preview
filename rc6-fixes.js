...
function rc6ApplyRainExposure(candidates,limit){
 ...
 const managed=rows.filter(store=>rc6OwnershipTier(store)<2),general=rows.filter(store=>rc6OwnershipTier(store)>=2),result=[];
 let managedIndex=0,generalIndex=0;
 for(let position=0;position<limit&&(managedIndex<managed.length||generalIndex<general.length);position+=1){
  const managedTarget=Math.round((position+1)*ratio),preferManaged=managedIndex<managedTarget;
...
function rc6ManagedStoreHeroEntries(){
 ...
 const candidates=stores.filter(store=>fxVisible(store)&&(rc6RainMode==='normal'?rc6OwnershipTier(store)<2:true));
