let rc6Coordinates={},rc6BannerTargets={},rc6StorePriority={},rc6HeroCampaigns={campaigns:{}},rc6ManagedStoreIds=new Set(),rc6SharedManagedStoreIds=new Set(),rc6Pointer=null,rc6LocationCache={key:'',stores:[]},rc6ChannelSortingInstalled=false,rc6HeroRenderKey='',rc6HeroDayTimer=0,rc6ActiveHeroDay='';
...
function rc6CampaignHeroEntries(){
 ...
 const photos=(campaign.images||[]).map((src,index)=>({
  banner:{desktop:src,mobile:src},index,key:`campaign-${store.id}-${index+1}`,target:{label:campaign.title||store.name},store,rankedStore:store,tier:0,kind:'store',presentation:'campaign-photo',campaignLabel:campaign.label,campaignTitle:campaign.title,campaignMeta:campaign.meta
 }));
