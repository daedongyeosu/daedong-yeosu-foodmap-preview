...
function rc6HeroLowFeeIcons(store){
 ...
 return keys.length?`<span class="rc6-hero-low-fee-icons" aria-label="이 가게에서 주문 가능한 저수수료 주문: ${escapeHtml(labels)}">${keys.map(key=>appIcon(key,'rc6-hero-low-fee-icon')).join('')}</span>`:'';
}
...
const rc6HeroDataPromise=Promise.all([fetchJson('data/store-coordinates.json',{}),fetchJson('data/banner-targets.json?v=notion-banner-15',{}),fetchJson('data/store-priority.json',{}),fetchJson('data/hero-campaigns.json?v=store-campaign-nine-2',{campaigns:{},virtualStores:{}}),rc6LoadRainMode()]);
