...
function rc6ConfigurePartnerPriority(){
 ...
 const profile=Object.hasOwn(profiles,RC6_PARTNER_KEY)?profiles[RC6_PARTNER_KEY]:null;
 rc6PartnerStoreIds=new Set(!RC6_IS_GOHEUNG&&profile?.enabled===true?(profile.storeIds||[]).map(String):[]);
}
...
function rc6OwnershipTier(store){const id=String(store?.id??store?.store_id??'');return rc6PartnerActive()?(rc6PartnerStoreIds.has(id)?0:rc6DeprioritizedStoreIds.has(id)?3:2):rc6ManagedStoreIds.has(id)?0:rc6SharedManagedStoreIds.has(id)?1:rc6DeprioritizedStoreIds.has(id)?3:2;}
...
async function rc6LoadRequestedCampaignMenuSlides(){
 ...
 // Curated batches must not be replaced by unreviewed/duplicate menu images.
 if(campaign.menuHydration==='curated-static'||staticSlides.length>=14)return;
 try{
...
function rc6RenderHero(){
 ...
 if(rc6HeroRenderKey===renderKey&&track.children.length)return;
 const viewportAnchor=fxCaptureDownstreamAnchor(hero);
 rc6HeroRenderKey=renderKey;if(heroCarousel)heroCarousel.destroy();
 ...
 fxRestoreDownstreamAnchor(viewportAnchor);
}
...
const rc6HeroDataPromise=Promise.all([fetchJson('data/store-coordinates.json',{}),fetchJson('data/banner-targets.json?v=native-pages-1',{}),fetchJson('data/store-priority.json?v=former-managed-bottom-1-baeknyeon-ended-1-shared-referral-2',{}),fetchJson('data/hero-campaigns.json?v=store-campaign-standard-1-auto-menu-fill-all-1-domino-munsu-14-1-four-store-qr-1-four-store-14-plus-3-1-teum-campaign-1-three-store-campaign-1-haeinine-campaign-1-collected-shopinshop-1-managed-business-1-ajuker-munsu-14-1-shared102-menu-1-store-entry-status-20260913-all-store-dedicated-1',{campaigns:{},virtualStores:{}}),rc6LoadRainMode()]);
