...
assert.match(html,/https:\/\/preview\.daedongmap\.com\//,'공식 공유·메타 주소는 현재 서비스 중인 .com 호스트여야 합니다.');
...
assert.match(finalExperience,/const FX_HOME_SHARE_URL='https:\/\/preview\.daedongmap\.com\/';/);
...
const shareUrl=new URL('https://preview.daedongmap.com/');
shareUrl.searchParams.set('store',sampleId);
assert.equal(shareUrl.origin,'https://preview.daedongmap.com');
assert.equal(shareUrl.searchParams.get('store'),sampleId);

console.log(JSON.stringify({homeShare:'https://preview.daedongmap.com/',sampleStore:sample.name,storeShare:shareUrl.href}));
