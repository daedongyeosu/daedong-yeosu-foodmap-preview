...
for(const [id,name] of approved){
  ...
  assert.equal(campaign.slides.length,14,`${name}: fourteen verified food photos are required`);
  assert.equal(new Set(campaign.slides.map(s=>s.image)).size,14);
  ...
  for(const slide of campaign.slides){
    ...
    assert.doesNotMatch(slide.meta,/\d[\d,]*\s*원|와우\s*회원|배달비/);
  }
